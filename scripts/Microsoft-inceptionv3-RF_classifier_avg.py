...
print("xceptionfeatures_shape:",inceptionv3features.shape)
...
plt.savefig('Inception_RF_avg_confusion_matrix.png', bbox_inches='tight', format='png')

#plt.show()

metrics_df.to_csv('RF_avg_inception.csv', index=False)
pd.DataFrame({'Average Accuracy': [avg_acc]}).to_csv('RF_avg_inception.csv', mode='a', index=False, header=True)
pd.DataFrame({'Execution time': [execution_time]}).to_csv('RF_avg_inception.csv', mode='a', index=False, header=True)
